...
def most_goals(players: list):
    players_list = sorted(players, key=lambda player: player.goals, reverse=True)
    return players_list[0].name

def most_points(players: list):
    individual = max(players, key=lambda player: (player.goals + player.passes))
    return (individual.name, individual.number)

def least_minutes(players: list):
    return min(players, key=lambda player: player.minutes)
    

# print(most_goals([BallPlayer("a",2,2,2,2), BallPlayer("b",1,1,1,1)]))
# print(least_minutes([BallPlayer("a",2,2,2,2), BallPlayer("b",1,1,1,1)]))
